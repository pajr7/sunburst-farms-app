...
export default function PostCard({ post, index = 0 }: { post: Post; index?: number }) {
  ...
  return (
    ...
    >
      ...
      <h3 className="font-semibold text-[15px] mb-1.5" style={{ color: "var(--charcoal)" }}>
        {post.title}
      </h3>
      <p className="text-sm leading-relaxed mb-3" style={{ color: "var(--stone)" }}>
        {post.body}
      </p>
      ...
      <div className="flex items-center gap-4 pt-1">
        <button
          ...
          style={{ color: liked ? "var(--terracotta)" : "var(--stone)" }}
        >
          <svg width="18" height="18" viewBox="0 0 24 24" fill={liked ? "var(--terracotta)" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
          </svg>
          {likeCount}
        </button>
        <button className="flex items-center gap-1.5 text-sm font-medium" style={{ color: "var(--stone)" }}>
